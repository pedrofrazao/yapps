...
class block(Agent):
    def __init__(self, arena=None):
        super().__init__( "B", state={}, arena=arena, volume=99, priority=0, can_move=False)
...
class prey(Agent):
    def __init__(self,state={}, priority=10, arena=None):
        super().__init__( f"p{prey.id}", AgentState(self,**state), priority=priority, arena=arena, volume=33, can_move=True )
        prey.id += 1
...
def arena_demo(rows,cols):
    ...
    for i in range(10):
        obj = prey( { 'energy':300, 'direction':9}, arena=arena)
        arena.add_to_position(0, i, obj)
