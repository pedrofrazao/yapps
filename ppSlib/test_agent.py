...
class TestSee(unittest.TestCase):
    def setUp(self):
        ...
        alist = []
        alist.append( Prey( arena = self.arena, dir=1, state_args=agent_attr, actions = [] ) )
        alist.append( Prey( arena = self.arena, dir=2, state_args=agent_attr, actions = [] ) )
        alist.append( Prey( arena = self.arena, dir=3, state_args=agent_attr, actions = [] ) )
        alist.append( Prey( arena = self.arena, dir=4, state_args=agent_attr, actions = [] ) )
    ...
    def test_see0(self):
        ...
        a1 = Prey( arena = self.arena, dir=1, state_args=a_attr, actions = [ see_debug() ] )
...
class TestPrey(unittest.TestCase):
    ...
    def test_prey(self):
        pa = [ move( params={ 'distance': 1, 'energy_penalty': 2, 'change_direction_prob': 0} )]
        p1 = Prey( arena = self.arena, dir=1, state_args={'energy': 10, 'epoch_penalty':1, 'see_length':0 }, actions = pa )
